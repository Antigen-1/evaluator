...
(begin-encourage-inline
  ...
  (define-generics amb-form
    ...
    #:fast-defaults ((default-representation?
                       ...
                       (define (lambda-any-arg f) (check-and-extract-form f (`(lambda (,_ ...) ,_ ...) #f) (`(lambda (,_ ... . ,arg) ,_ ...) arg) (`(lambda ,arg ,_ ...) arg)))
             ...
             (plain-expand
              (lambda (f e)
                ...
                (cond ((let ((expanded (env-expand f e)))
                      ...
                      ((lambda? f) (make-lambda `(,@(n:lambda-fixed-args f) . ,(n:lambda-any-arg f))
...
(module* test racket/base
  ...
  (check-equal? (expand-amb '(let ((a 1)) (+ a 1)) env) '((lambda (a . #f) (+ a 1)) 1))
  (check-equal? (expand-amb '(let* ((a 1) (b (+ a 1))) (+ a b)) env) '((lambda (a . #f) ((lambda (b . #f) (+ a b)) (+ a 1))) 1))
